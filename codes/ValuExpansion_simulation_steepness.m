% sigmoidfit_basedonOV_Task1simulate.m
%
...
for imonkey = 1:nmonkeys
    ...
    for isess = 1:nsess_final
        ...
        mdl1 = fitglm(log(QB_SO./QA_SO), PchoB_SO, 'linear', 'Distribution', 'binomial', 'link','probit');
        ...
        PchoBn = predict(mdl1,real(log(QBn./QAn)));
        %
%         if doRandomResample
...
%         end
        ...
        mdl2 = fitglm(log(QB_JC./QA_JC), PchoBn, 'linear', 'Distribution', 'binomial', 'link','probit');
...
%     %
    ...
    %
    ynames = {'\rho Task1 simulated', '\rho Task1 simulated', '\eta Task1 simulated', '\eta Task1 simulated'};
...
%     %
%     ynames = {'\eta Task1 simulated', '\eta Task1 simulated'};
%     xnames = {'\eta Task1', '\eta Task2'};
    ...
    for iplot = 1:nplots
        ...
        %
        eval(['XXX = ',xplottypes{iplot},';'])
        eval(['YYY = ',yplottypes{iplot},';'])
        %
    ...
    %
    subplot(1,1,1);
    XXX = rho_fit_basedonOV - rho_JC;
%     YYY = 2*(rho_SO - rho_JC)./(rho_SO + rho_JC);
    YYY = (rho_SO - rho_JC);
    %
%     ind_bad = XXX>0.6;
%     XXX(ind_bad) = [];
