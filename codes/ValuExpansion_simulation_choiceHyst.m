% sigmoidfit_basedonOV_Task1simulate_choiceHyst_eachtrtial.m
%
...
%         mdl_SO = fitglm([log(QB_SO(ii_SO)./QA_SO(ii_SO)),ChHyst_SO(ii_SO),ord_SO(ii_SO)], CJ_SO(ii_SO), 'Distribution','binomial', 'link','probit');
        mdl_SO = fitglm([log(QB_SO(ii_SO)./QA_SO(ii_SO)),ChHyst_SO(ii_SO)], CJ_SO(ii_SO), 'Distribution','binomial', 'link','probit');
        
        % step 3: update QA and QB
        % % q1* = bb*q2 + (1-bb)*range(q)/2
        QAn = (1./bb)*QA_JC + (1-(1./bb))*rangeA_JC/2;
        QBn = (1./bb)*QB_JC + (1-(1./bb))*rangeB_JC/2;
       
        
        % step 4: based on the fitting in step 2, get the new PchoBn
        % simulate for each trial
        PchoBn = predict(mdl_SO,[real(log(QBn./QAn)),ChHyst_JC]);
        ntrials = length(PchoBn);
        CJ_JCsim = binornd(1,PchoBn);
   
        % step 5: redo the new fitting for JC
        mdl_JCsim = fitglm([log(QB_JC(ii_JC)./QA_JC(ii_JC)),ChHyst_JC(ii_JC)], CJ_JCsim(ii_JC), 'Distribution','binomial', 'link','probit');
        
        ind_goodfit(isess,1) = 1;
        
        % 
        % summarize the results
        rho_JCsim_withHyst(isess,1) = exp(-mdl_JCsim.Coefficients.Estimate(1)/mdl_JCsim.Coefficients.Estimate(2));
        steepness_JCsim_withHyst(isess,1) = mdl_JCsim.Coefficients.Estimate(2);
        hyst_JCsim_withHyst(isess,1) = 2*rho_JCsim_withHyst(isess,1)* mdl_JCsim.Coefficients.Estimate(3)/mdl_JCsim.Coefficients.Estimate(2);
        %
...
%     set(gcf,'position',[110 65 1050 550], 'PaperPositionMode','auto')
    ...
    axis off
%     %
    yplottypes = {'hyst_JCsim_withHyst', 'hyst_JCsim_withHyst', 'hyst_SO_withHyst'};
    xplottypes = {'hyst_JC_withHyst', 'hyst_SO_withHyst', 'hyst_JC_withHyst'};
    %
    ynames = {'\xi Task1 simulated with hysteresis', '\xi Task1 simulated with hysteresis', '\xi Task2 with hysteresis'};
...
%     set(gcf,'position',[110 65 1050 550], 'PaperPositionMode','auto')
    ...
    axis off
%     %
    yplottypes = {'rho_JCsim_withHyst', 'rho_JCsim_withHyst', 'steepness_JCsim_withHyst', 'steepness_JCsim_withHyst'};
    xplottypes = {'rho_JC_withHyst', 'rho_SO_withHyst', 'steepness_JC_withHyst', 'steepness_SO_withHyst'};
    %
    ynames = {'\rho Task1 simulated with hysteresis', '\rho Task1 simulated with hysteresis', '\eta Task1 simulated with hysteresis', '\eta Task1 simulated with hysteresis'};
...
%         ind_goodY = steepness_SO<(mean(steepness_SO)+3*std(steepness_SO)) & steepness_SO>(mean(steepness_SO)-3*std(steepness_SO));
    ...
    steepness_delta_sim = steepness_SO_withHyst - steepness_JCsim_withHyst;
    hyst_delta_data = hyst_SO_withHyst - hyst_JC_withHyst;
    hyst_delta_sim = hyst_SO_withHyst - hyst_JC_withHyst;
    %
